-- Analytics: member-scoped multi-store snapshot.
-- Same digest as admin_store_snapshot, but visible to store admins for the
-- stores they belong to (memberships), not just superadmins. Superadmins keep
-- seeing every store. Used by the Admin-only Analytics page.
--
-- RLS blocks cross-tenant reads from the client, so this runs as security
-- definer and filters to: member tenants OR (superadmin -> all tenants).

create or replace function public.my_stores_snapshot()
returns table (
  id uuid,
  business_name text,
  revenue_30d numeric,
...
revoke all on function public.my_stores_snapshot() from public;
grant execute on function public.my_stores_snapshot() to authenticated;
